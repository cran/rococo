...
Note that the \RoCoCo\ package performs only a few basic checks on a user-defined
t-norm. It remains the duty of the user to ensure that the function is
actually a valid t-norm.
...
drastic slowdown of computations, particularly for larger data sets.
...
<<LargeNoShuffles>>=
rococo.test(x2, y2, numtests=10000)
@
