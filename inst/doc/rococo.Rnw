...
\begin{enumerate}
\item Download \texttt{rococo\_\RoCoCoVer.zip}
...
{\ttfamily rococo\_\RoCoCoVer.zip} and select this file. The
...
\item Open a shell window and change to the directory where you put
  {\ttfamily rococo\_\RoCoCoVer.tar.gz}. Enter
\begin{quote}
...
without severe problems on \R\ versions $\geq$2.12.0.
...
\begin{center}
<<ToyData,fig=TRUE,echo=TRUE,width=5,height=5>>=
x1 <- rnorm(15)
...
\begin{center}
<<RoCoCoTestFormula,fig=TRUE,echo=TRUE,width=5,height=5>>=
data(iris)
...
\begin{center}
<<FlatNoisyData,fig=TRUE,echo=TRUE,width=5,height=5>>=
x2 <- rnorm(15)
f2 <- function(x)
{
    if (x > 0.8) x - 0.8
...
analytic deduction of the test statitistic's distribution.
However, as extensive experiments have shown, we can assume that the test
statistic is approximately normally distributed around 0, where only the
standard deviation depends on the data distribution and the parameter
settings of the rank correlation measures.
Therefore, we use {\em permutation testing} for estimating the
standard deviation of the test statistic under the assumption of independence.
...
Then we create $K$ random shuffles of $(y'_i)_{i=1}^n$ and compute
...
with given marginal distributions and we can estimate the test's $p$-value by
the resulting normal distribution. Assume that $\tilde\gamma$ is the observed
gamma rank and that $\tilde\sigma$ is the estimated standard deviation from
0 computed from the observed test statistics on the random shuffles.
Then the $p$-values can be computed as ($P_{\mu,\sigma}$ denotes the cummulative
distribution function of the normal distribution with mean $\mu$ and
variance $\sigma^2$)
\begin{itemize}
  \item $2\cdot(1-P_{0,\tilde\sigma}(|\tilde\gamma|))$ in case we perform
    a two-sided test,
  \item $1-P_{0,\tilde\sigma}(\tilde\gamma)$ in case we perform a
    one-sided test with alternative hypothesis that there is a positive
    correlation, and
  \item $P_{0,\tilde\sigma}(\tilde\gamma)$ in case we perform a
    one-sided test with alternative hypothesis that there is a negative
    correlation.
\end{itemize}
This $p$-value is stored in the slot \texttt{p.value} of the resulting
output object of \texttt{rococo.test()}. As usual in \R,
$<\Sexpr{format(.Machine$double.eps, digits=2)}$ is displayed if the $p$-value
is lower than machine precision. Moreover, the output object also contains
a slot \texttt{count} which corresponds to the number of times
\begin{itemize}
  ...
  \item the test statistic was greater than
   ...
   \item the test statistic was less than
    ...
    correlation.
\end{itemize}
This count allows for an alternative method of determining a $p$-value as
a relative frequency. Users who want to use this method can compute the
$p$-value as the quotient of the above count (slot \texttt{count})
over the number of trials (slot \texttt{numtests}).

The number of shuffles performed by \texttt{rococo.test()} is controlled by
the parameter \texttt{numtests} (the default is 1000). It is clear that,
the smaller the number of shuffles, the less exact the estimation is.
...
all random shuffles:
\begin{center}
<<LargeNoShuffles,fig=TRUE,echo=TRUE,width=5,height=5>>=
...
plot(function(x) dnorm(x, mean=0, sd=res@H0gamma.sd), min(res@perm.gamma),
     max(res@perm.gamma), col="red", lwd=3, add=TRUE)
@
\end{center}
The histogram and the density of the corresponding normal distribution
(red curve) match almost perfectly.
...
%ConnectivesOperators,FuzzySetsRelations,ComputerScienceMisc}
